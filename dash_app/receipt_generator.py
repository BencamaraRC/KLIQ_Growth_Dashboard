...
# ── Company Details ──
COMPANY_NAME = "Remote Coach Ltd Trading as KLIQ"
...
def generate_receipt_pdf(
    ...
    header_data = [
        [
            ...
            Paragraph(
                f"Remote Coach Ltd Trading as KLIQ<br/>{COMPANY_ADDRESS.replace(chr(10), '<br/>')}<br/>"
                f"{COMPANY_EMAIL}",
                styles["CompanyInfo"],
            ),
        ]
    ...
    elements.append(
        Paragraph(
            f"Remote Coach Ltd Trading as KLIQ · {COMPANY_ADDRESS.replace(chr(10), ' · ')} · {COMPANY_EMAIL} · {COMPANY_WEB}",
            styles["FooterText"],
        )
    )
